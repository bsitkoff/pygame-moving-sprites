# This Assignment: Lesson 2 — Moving Sprites

First PRIMM lesson. Students explore `move.py` (keyboard input, one direction works, others are TODOs), then build their own in `tryit.py`.

**Student files**: `move.py` (example), `tryit.py` (student work), `images/`
**Key concepts**: Actor creation, keyboard input in update(), boundary checking
